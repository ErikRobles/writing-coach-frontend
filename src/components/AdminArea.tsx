...
export default function AdminArea() {
  ...
  useEffect(() => {
    if (role !== 'admin') {
      setError('Unauthorized clearance level. Terminal locked.');
      return;
    }
    fetch('http://localhost:8080/admin/users', {
      ...
      .then(r => {
        if (!r.ok) throw new Error('Failed to retrieve participant roster');
        return r.json();
      })
  ...
  return (
    <div className="p-8 md:p-12 w-full max-w-7xl mx-auto space-y-12">
      <header className="space-y-4">
        ...
        <p className="text-on-surface-variant">Global Participant Telemetry & Access Control Roster</p>
      </header>
      ...
      <section className="bg-surface/20 border border-on-surface/5 rounded-3xl overflow-hidden backdrop-blur-md">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface/50 border-b border-on-surface/10 font-space-grotesk">
                <th className="p-4 text-xs tracking-wider uppercase text-on-surface-variant font-bold">Participant ID</th>
                <th className="p-4 text-xs tracking-wider uppercase text-on-surface-variant font-bold">Clearance</th>
                <th className="p-4 text-xs tracking-wider uppercase text-on-surface-variant font-bold">Interventions</th>
                <th className="p-4 text-xs tracking-wider uppercase text-on-surface-variant font-bold">Streak</th>
                <th className="p-4 text-xs tracking-wider uppercase text-on-surface-variant font-bold">Registration Data</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-on-surface/5">
              {users.map((u, i) => (
                <tr key={i} className="hover:bg-surface/30 transition-colors">
                  <td className="p-4">
                    <div className="font-space-grotesk font-bold text-on-surface">{u.email || u.username || 'Unknown Identity'}</div>
                    <div className="font-inter text-xs text-on-surface-variant/50 max-w-[120px] truncate">{u.user_id}</div>
                  </td>
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${
                      (u.role || 'user') === 'admin' ? 'bg-emerald-400/10 text-emerald-400 border border-emerald-400/20' : 'bg-primary/10 text-primary border border-primary/20'
                    }`}>
          ...
          {users.length === 0 && <div className="p-8 text-center text-on-surface-variant">Fetching participant metrics...</div>}
        </div>
      </section>
    </div>
  );
